...
describe('Generics 01 - createBuilder (core)', () => {
  ...
  it('type-checks correctly with good inference (type level)', () => {
    ...
    expectTypeOf<typeof result>().toEqualTypeOf<{ id: string; count: number }>();
    expectTypeOf<(typeof result)['id']>().toEqualTypeOf<string>();
    expectTypeOf<(typeof result)['count']>().toEqualTypeOf<number>();

    // @ts-expect-error - setting a key that does not exist on the shape must be an error
    builder.set('unknownKey', 123);

    // Partial<Shape> tradeoff chosen (see src/builder.ts header comment): .build() is always
    // allowed at type level. If a required field is added later to the Shape used at a call site,
    // old .build() calls will NOT break at compile (runtime may miss it). Documented decision.
  });
  ...
  it('supports being used inside pure functions as well as class methods', () => {
    ...
    expectTypeOf<typeof pureUser>().toEqualTypeOf<{ id: string; role: 'user'; createdAt: Date }>();
    expectTypeOf<typeof oopUser>().toEqualTypeOf<{
      id: string;
      name: string;
      type: 'admin' | 'member';
    }>();
