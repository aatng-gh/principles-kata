...
- A final `.build()` returning the exact `Shape` (or `Partial<Shape>` per your documented choice). Full SetKeys tracking can make incomplete `.build()` a compile error; the Partial choice (common for fluency) is acceptable if the tradeoff is clearly documented in code + comments.
...
- Adding a new required field to `Shape` should cause a compile error at the `.build()` call site (or on use of the result) if the field was never `.set()` *if using SetKeys tracking*; the common practical choice is `Partial<Shape>` for build (always allowed at type level) with clear documentation of the tradeoff (see src impl comment and test). Both are acceptable if documented.
...
- The implementation may use an accumulating `Builder<Shape, SetK>` (for full required tracking) *or* a clean `Partial<Shape>` approach; in either case document the design/tradeoff in a src comment. The public API must remain precise and support dual OOP/FP use.
