...
describe('Type Guards 02 - Event guards in mixed OOP/FP', () => {
  it('isAppEvent narrows in if and in filter (types)', () => {
    ...
    const mixed: unknown[] = [{ type: 'key', key: 'Enter' }, 123, { type: 'mouse', x: 0, y: 0 }];
  ...
  it('class processor and pure collector both rely on the same guard without casts', () => {
    ...
    // last is set inside handle when guard passed; use runtime guard for type-narrow in test (no ! or cast)
    if (proc.last !== undefined) {
