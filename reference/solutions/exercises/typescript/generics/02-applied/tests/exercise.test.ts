...
describe('Generics 02 - withCache (applied dual paradigm)', () => {
  ...
  it('works on class methods while preserving method signature and this', () => {
    ...
    const cachedCompute = withCache(calc.compute.bind(calc)); // or handle this inside if your impl supports it

    expectTypeOf(cachedCompute).toEqualTypeOf<(x: number, y: number) => number>();

    const a = cachedCompute(3, 4);
    const b = cachedCompute(3, 4);
